...
mask_files = glob.glob(f'{ANN_DIR}/*-mask.png')
annotated_files = glob.glob(f'{ANN_DIR}/*-annotated.png') + glob.glob(f'{ANN_DIR}/*-annotate.png')
...
for m_path in mask_files:
    ...
    print(f"  ✓ {base.split('/')[-1]} (-mask): {(mask > 0).mean():.1%} of frame is board")

    # Save debug visualization
    debug = orig.copy()
    debug[mask > 0] = (debug[mask > 0] * 0.45 + np.array([0, 200, 0]) * 0.55).astype(np.uint8)
    out_name = base.split('/')[-1] + '_gt.jpg'
    cv2.imwrite(str(DEBUG_DIR / out_name), debug)
...
for a_path in annotated_files:
    ...
    pairs.append((orig, mask))
    processed_bases.add(base)
    print(f"  ✓ {base.split('/')[-1]} (-annotated): {(mask > 0).mean():.1%} of frame is board")
...
config = {
    ...
    "threshold":     0.18,
    "trained_on_frames": list(processed_bases),
    "epochs": EPOCHS,
