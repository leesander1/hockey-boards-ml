...
def stitch_affine(video_path, output_path):
    ...
    for y in range(450):
        for x in range(800):
            dist_x = min(x, 800-x-1)
            dist_y = min(y, 450-y-1)
            dist = min(dist_x, dist_y)
            if dist < fade_px:
                feather_mask[y, x, 0] = max(dist / fade_px, 0.01)
    ...
    # FIX for NumPy array assignment
    tmp_result = np.zeros_like(accumulator)
    np.divide(accumulator, weight_map, out=tmp_result, where=weight_map > 0)
    result = tmp_result.astype(np.uint8)
