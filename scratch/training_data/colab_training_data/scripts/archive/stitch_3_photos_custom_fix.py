...
def main():
    ...
    # Make canvas wide enough just in case
    canvas_w = int(w * 4)
    canvas_h = int(h * 3)
    offset_x = int(w * 1.5)
    offset_y = h
    ...
    def get_homography(idx_src, idx_dst):
        ...
        print(f"Good matches {idx_src} -> {idx_dst}: {len(good_matches)}")
    ...
    # FIX: Use findNonZero to get bounding box of ALL pixels, not just contours[0]
    ...
    output_path = "/Users/leesander/.gemini/antigravity/brain/d5aae78c-4b02-47aa-b503-4927926f04c0/artifacts/stitched_3_photos_aligned.jpg"
    cv2.imwrite(output_path, result)
    print(f"Saved aligned 3-photo stitch to {output_path}")
